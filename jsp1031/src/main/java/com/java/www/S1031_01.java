...
public class S1031_01 extends HttpServlet {
	...
	protected void doAction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("id : "+request.getParameter("id")); // console 창에 값 보여줌
		System.out.println("hobby : "+Arrays.toString(request.getParameterValues("hobby"))); // <- 배열의 값 찍을 때 Arrays.toString() 사용
		//배열을 hobbys 변수선언
		...
		// db에는 배열을 저장할 수 없기 때문에 위의 작업을 해줘야 함
		
		
		writer.println("<html>");		
		writer.println("<head><title>form데이터</title></head>");		
		writer.println("<body>");		
		writer.println("checkbox hobby : "+Arrays.toString(request.getParameterValues("hobby")));
		writer.println("<br>");
